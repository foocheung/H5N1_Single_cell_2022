...
for(i in 1:length(celltypes_sxmd.DeltaDelta)){
  ...
  cont.matrix.DeltaDelta[[i]] <- makeContrasts( 
    ...
    g1=GroupNEG_D100-GroupNEG_D21,
    g2=GroupNEG_D100-GroupNEG_D0,
    
    g3=GroupPOS_D100-GroupPOS_D21,  
    g4=GroupPOS_D100-GroupPOS_D0,  
...
##btm = readRDS(file = "./Genesets/BTM_li.rds")
#btm = gmtPathways("./Genesets/patterns.gmt")
btm= gmtPathways("./Genesets/adv_D22_d21.gmt")
...
do.call(rbind, celltypes_score.DeltaDelta1) %>% 
  ...
  write_tsv('DE_wctcelltype_QCd/figure_output/module_heatmaps.DeltaDelta/le_patterns_neg_adj_D100-D21.tsv')
...
do.call(rbind, celltypes_score.DeltaDelta2) %>% 
  rowwise() %>% 
  mutate_if(is.list, ~paste(unlist(.), collapse = ',')) %>% 
  write_tsv('DE_wctcelltype_QCd/figure_output/module_heatmaps.DeltaDelta/le_patterns_neg_adj_D100-D0.tsv')
...
do.call(rbind, celltypes_score.DeltaDelta3) %>% 
  rowwise() %>% 
  mutate_if(is.list, ~paste(unlist(.), collapse = ',')) %>% 
  write_tsv('DE_wctcelltype_QCd/figure_output/module_heatmaps.DeltaDelta/le_patterns_pos_adj_D100-D21.tsv')
...
do.call(rbind, celltypes_score.DeltaDelta4) %>% 
  rowwise() %>% 
  mutate_if(is.list, ~paste(unlist(.), collapse = ',')) %>% 
  write_tsv('DE_wctcelltype_QCd/figure_output/module_heatmaps.DeltaDelta/le_patterns_pos_adj_D100-D0.tsv')
